...
/**
 * Created by YUNUS on 05.03.2017.
 */
public class Helper {
    ...
    public static String CreateMopFile(SaidaContract contract, File source, File dest) throws IOException {
        ...
        for (Event event:contract.getEvents()) {
            ...
            switch (event) {
                case FieldGet:sb.append(String.format("get(@FIELDINFO);"));
                    break;
                case FieldSet:sb.append(String.format("set(@FIELDINFO);"));
                    break;
                case MethodCall:sb.append(String.format("call(@METHODINFO);"));
                    break;
            }
        }
        sb.append("\nltl: ");
        switch (contract.getTemporalOperator()) {
            case Never:sb.append("![]");
                break;
            case Always:sb.append("[]");
                break;
        }
        if (eventNames.size()==1){
            ...
            switch (contract.getTemporalOrder()) {
                case Before:sb.append(String.format("{0} => {1}",eventNames.get(0),eventNames.get(1)));
                    break;
                case After:sb.append(String.format("{0} => {1}",eventNames.get(1),eventNames.get(0)));
                    break;
            }
        }
